...
const OcCategoWidget = {
    ...
    canRemoveCategory(widget, categoryItem) {
        ...
        const addedDate = categoryItem.dataset.oc_catego_asignado_el;
...
// Update move buttons based on permissions
    ...
    initAll() {
        ...
        console.log('✅ ocCatego,initAll(): initialized', this.version);
        return initialized;
    },
    ...
    updateItemButtonAfterDrop(item, targetList) {
        ...
        if (isInSelectedList) {
            ...
            button.setAttribute('onclick', "OcCategoWidget.moveCategory(this, 'right')");
            ...
            button.setAttribute('onclick', "OcCategoWidget.moveCategory(this, 'left')");
    ...
    handleInput: function(event) {
        ...
        if (widget) {
            OcCategoWidget.filterCategories(widget, event.target.value);
        }
    },
    ...
    handleKeydown: function(event) {
        if (event.key === 'Escape') {
            ...
            if (widget) {
                event.target.value = '';
                OcCategoWidget.filterCategories(widget, '');
            }
        }
    },
    ...
    initializeFromLinkedSelect(widget) {
        ...
        allOptions.forEach(option => {
            ...
            const categoryItem = this.createCategoryItem(widget, option.value, option.textContent, isSelected);
    ...
    syncSelectSelectedToWidgetMALOBORRAME: function(widget) {
        const selectId = widget.dataset.oc_catego_linked_select;
        if (!selectId) return;
        const selectEl = document.getElementById(selectId);
        if (!selectEl) return;
    ...
    syncFromSelect: function(selectorWidget, linkedSelect) {
        const assignedContainer = selectorWidget.querySelector('.oc_catego_si');
        const availableContainer = selectorWidget.querySelector('.oc_catego_disponibles');
        ...
        const selected = new Set(Array.from(linkedSelect.selectedOptions).map(o => o.value));
        const allOptions = Array.from(linkedSelect.options);

        for (const opt of allOptions) {
            const assigned = selected.has(opt.value);
            const item = OcCategoWidget.createCategoryItem(opt.value, opt.textContent, assigned, selectorWidget);
            if (assigned) {
     ...
     */
    createCategoryItem(widget, id, label, isSelected = false) {
        let dragHandle,moveButton;
        if(this.isReadOnly(widget)) {
            moveButton= dragHandle = '';
        } else {
            dragHandle = '<div class="oc_catego_drag-handle">⇔</div>';
            ...
            moveButton = `<button class="oc_catego_arrow-button ${arrowClass}" 
                type="button" onclick="OcCategoWidget.moveCategory(this, '${moveDirection}')"
                title="${arrowTitle}"
            > ${arrowSymbol} </button>
            `;
        }
    ...
    updateWidgetCategories(widget, newCategories) {
        ...
        newCategories.forEach(category => {
            ...
            const categoryItem = this.createCategoryItem(widget, category.id, category.label, isSelected);
            targetList.appendChild(categoryItem);
        });
    ...
    syncFromSelect: function(selectorWidget, linkedSelect) {
        ...
        // Create maps for selection
        const selectedValues = new Set(Array.from(linkedSelect.selectedOptions).map(opt => opt.value));
        const allOptions = Array.from(linkedSelect.options);

        for (const opt of allOptions) {
            const item = document.createElement('div');
            item.className = 'oc_catego_item';
            item.setAttribute('data-value', opt.value);
            item.setAttribute('role', 'option');

            const name = document.createElement('div');
            name.className = 'oc_catego_name';
    ...
    moveCategory(button, direction) {
        ...
        if (direction === 'right') { // Removing from selected
            ...
            const today = new Date().toISOString().split('T')[0];
            categoryItem.dataset.oc_catego_asignado_el = today;
        }
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    if (direction === 'left') {
                        ...
                        arrowButton.setAttribute('onclick', "OcCategoWidget.moveCategory(this, 'right')");
                        arrowButton.setAttribute('title', 'Remover categoría');
                    } else {
                        arrowButton.innerHTML = '←';
                        arrowButton.className = 'oc_catego_arrow-button oc_catego_arrow-button--left';
                        arrowButton.setAttribute('onclick', "OcCategoWidget.moveCategory(this, 'left')");
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => OcCategoWidget.initAll());
} else {
    OcCategoWidget.initAll();
}
