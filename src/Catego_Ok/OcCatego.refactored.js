...
class OcCatego {
    ...
    async _save(id) {
        ...
        const normalize = (str) => (typeof OcSelectUtil !== 'undefined' && typeof this.ocSelectUtil._normalize === 'function')


            ? this.ocSelectUtil._normalize(String(str))


            : this._normalizeString(String(str));


        // Duplicate check when we have a select


        if(hasSelect) {


            const newNorm = normalize(label);


            const dup = Array.from(this.selectElement.options || []).some(opt => {


                if(!opt.value) return false;


                const sameId = String(opt.value) === String(id);


                const sameLabel = normalize(opt.text) === newNorm;
        ...
        if(hasApi && !hasSelect) {
            ...
            if(res && res.ok === true) {
                ...
                this.clearEdit && this._clearEdit();


                this._showSuccess && this._showSuccess('Updated!');
        ...
        if(!hasApi && hasSelect) {
            ...
            try {
                ...
                if(typeof OcSelectUtil !== 'undefined' && typeof this.ocSelectUtil.selectEdit === 'function') {


                    this.ocSelectUtil.selectEdit(this.selectElement, String(id), label);


                    if(typeof this.ocSelectUtil.updateSelect === 'function') {


                        this.ocSelectUtil.updateSelect(this.selectElement);
                ...
                const msg = 'Error desconocido del servidor';
    ...
    async _add() {
        ...
        const raw = (this.searchInput ? this.searchInput.value : '').trim().replace(/\s+/g, ' ');
        ...
        if(hasSelect) {
            ...
            const dup = opts.some(opt => opt.value && normalize(opt.text) === newNorm);
        ...
        if(hasApi && !hasSelect) {
            ...
            if(res && res.ok === true) {
                ...
                // Update local view if applicable
                ...
                this._showSuccess && this._showSuccess('Added!');
        ...
        if(!hasApi && hasSelect) {
            ...
            try {
                ...
                if(typeof OcSelectUtil !== 'undefined' && typeof this.ocSelectUtil.selectAdd === 'function') {


                    this.ocSelectUtil.selectAdd(this.selectElement, {value: newId, label, selected: false});
                ...
                if(Array.isArray(this.categories)) {
                    ...
                    this.categories.push({id: newId, name: label});
        ...
        if(res && res.ok === true && res.id != null) {
            ...
            try {
                ...
                if(typeof OcSelectUtil !== 'undefined' && typeof this.ocSelectUtil.selectAdd === 'function') {
                    ...
                    this.ocSelectUtil.selectAdd(this.selectElement, {value: String(res.id), label, selected: false});
                ...
                console.log(msg, {accion: 'add', label, res}, err);


            }


            // Keep local list aligned
    ...
    async _delete(id) {
        ...
        if(hasApi) {
            ...
            if(res && res.ok === true) {
                if(hasSelect)
                    try {
                        this.this.ocSelectUtil.optionDelete(String(id));
                    } catch(err) {
                        const msg = 'La operación se realizó en el servidor, pero no se pudo actualizar el select.';
                        this._showError(msg);
