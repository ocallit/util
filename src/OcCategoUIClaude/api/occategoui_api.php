...
global $gSqlExecutor;
$tagsDbSchema =  [
  "CREATE TABLE IF NOT EXISTS ocCatalog(
  ...
  "CREATE TABLE IF NOT EXISTS ocCatalogTags(
    ...
    "CREATE TABLE IF NOT EXISTS ocCatlogTagged (
    ...
    switch ($action) {
        case 'tagList':
            ...
            $tags = $gSqlExecutor->array(
                "SELECT ocCatalogTags_id as id, tag as text 
                 FROM ocCatalogTags 
                 WHERE ocCatalog_id = ? 
                 ORDER BY tag",
                [$catalog_id]
        ...
        case 'tagAdd':
            ...
            try {
                $gSqlExecutor->query(
                    "INSERT INTO ocCatalogTags (ocCatalog_id, tag) VALUES (?, ?)",
                    [$catalog_id, $text]
                );
        ...
        case 'tagUpdate':
            ...
            $belongsToCatalog = $gSqlExecutor->firstValue(
                "SELECT COUNT(*) 
                 FROM ocCatalogTags 
                 WHERE ocCatalogTags_id = ? AND ocCatalog_id = ?",
                [$id, $catalog_id]
            );
            ...
            try {
                $gSqlExecutor->query("UPDATE ocCatalogTags SET tag = ? WHERE ocCatalogTags_id = ?", [$text, $id]);
                sendResponse(true);
            } catch (Exception $e) {
                if ($gSqlExecutor->is_last_error_duplicate_key()) {
        ...
        case 'tagDelete':
            ...
            $isUsed = $gSqlExecutor->firstValue("SELECT COUNT(*) FROM ocCatlogTagged WHERE ocCatalogTags_id = ?", [$id]);
            if ($isUsed > 0) {
                sendResponse(false, "Cannot delete tag - it's being used in $isUsed items");
            }

            try {
                $gSqlExecutor->query("DELETE FROM ocCatalogTags WHERE ocCatalogTags_id = ?", [$id]);
                sendResponse(true);
            } catch (Exception $e) {
