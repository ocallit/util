...
class StateMachine {
    ...
    protected function moveToAllowed( string $toState):bool {
        ...
        foreach($this->moveToGuard as $guard)
            if(!call_user_func($guard, $this->currentState, $toState, $this->luggage, "moveToGuard"))
                return false;
        foreach($current[StateMachine::GUARD_LEAVE] ?? [] as $guard)
            if(!call_user_func($guard, $this->currentState, $toState, $this->luggage, StateMachine::GUARD_LEAVE))
                return false;
        foreach($current[StateMachine::TRANSITION_TO][$toState][StateMachine::GUARD_TRANSITION] ?? [] as $guard)
            if(!call_user_func($guard, $this->currentState, $toState, $this->luggage, StateMachine::GUARD_TRANSITION))
                return false;
        if(!array_key_exists($toState, $this->states))
            return false;
        foreach($this->states[$toState][StateMachine::GUARD_ENTER] ?? [] as $guard)
            if(!call_user_func($guard, $this->currentState, $toState, $this->luggage, StateMachine::GUARD_ENTER))
                return false;
        return true;
    }
    ...
    protected function move(string $toState):void {
        ...
        foreach($this->onBeforeTransition as $on)
            call_user_func($on, $currentState, $toState, $this->luggage, "ON_BEFORE_TRANSITION");

        $current = $this->states[$this->currentState];
        foreach($current[StateMachine::ON_LEAVE] ?? [] as $on)
            call_user_func($on, $this->currentState, $toState, $this->luggage, StateMachine::ON_LEAVE);

        foreach($current[StateMachine::TRANSITION_TO][$toState][StateMachine::ON_TRANSITION] ?? [] as $on)
            call_user_func($on, $this->currentState, $toState, $this->luggage, StateMachine::TRANSITION_TO);

        $this->currentState = $toState;

        foreach($this->states[$toState][StateMachine::ON_ENTER] ?? [] as $on)
            call_user_func($on, $this->currentState, $toState, $this->luggage, StateMachine::ON_ENTER);

        foreach($this->onAfterTransition as $on)
            call_user_func($on, $currentState, $toState, $this->luggage, "ON_AFTER_TRANSITION");
     ...
     */
    public function nextStates():array { return array_keys($this->states[$this->currentState][StateMachine::TRANSITION_TO] ?? []); }
    ...
    public function getLuggage(): mixed {return $this->luggage;}
