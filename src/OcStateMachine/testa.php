...
<style>
    ...
    #stage { border:1px solid #ddd; border-radius:10px; padding:8px; height:70vh; overflow:auto; }
    #stage svg { width:100%; height:auto; } /* responsive */
</style>
...
<script>
    ...
    async function render() {
        ...
        try {
            ...
            if (pz) { pz.destroy(); pz = null; } // reset pan/zoom after re-render
        } catch (e) {
    ...
    document.getElementById('btnFields').onclick  = () => { hideF = !hideF; render(); };
    document.getElementById('btnMethods').onclick = () => { hideM = !hideM; render(); };
    document.getElementById('btnOrient').onclick  = () => { vertical = !vertical; render(); };
    document.getElementById('btnFit').onclick     = () => {
        const st = document.getElementById('stage'); st.scrollTop = 0; st.scrollLeft = 0;
    };
    document.getElementById('btnPZ').onclick      = () => {
        const svg = document.querySelector('#stage svg');
        if (!svg) return;
        if (pz) { pz.destroy(); pz = null; }
        else { pz = svgPanZoom(svg, { zoomEnabled:true, controlIconsEnabled:true, fit:true, center:true }); }
    };
