...
// Original Version: 2.0.0 - Migrated to DialogIt modal system
...
class TagIt {
    ...
    showAlert(message) {
        return DialogIt.alert(this.escapeHtml(message));
    }

    showConfirm(message) {
        return DialogIt.confirmDelete(
    ...
    openDialog() {
        ...
        // *** BUG FIX ***
        // When DialogIt closes with keepHtml:true, it sets the content to 'display: none'.
        ...
        const { dialog, promise } = DialogIt.dialog({
    ...
    filterOptions(searchTerm) {
        ...
        items.forEach(item => {
            const text = item.querySelector('.tagit_optionText').textContent.toLowerCase();
