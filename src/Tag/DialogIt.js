// File:DialogIt/DialogIt.js
// Version: 2.0.0

/**
 * DialogIt - Unified Dialog System
    ...
    dialog({title = "", html, buttons = [], keepHtml = false}) {
        ...
        const promise = new Promise((resolve, reject) => {
            ...
            function cleanup(shouldResolve = false, data = null) {
                ...
                DialogItDrag.cleanup(dialog);
                dialog.remove();

                if (shouldResolve) {
                    resolve(data);
                } else {
                    reject(new Error(DialogIt.CANCELED));
                }
            }
            ...
            DialogItDrag.initialize(dialog);
            dialog.showModal();
    ...
    utils: {
        ...
        symbolRowInit() {
            ...
            document.querySelectorAll('.symbols-row').forEach(row => {
                row.removeEventListener('click', DialogIt.utils.symbolRowClick);
                row.addEventListener('click', DialogIt.utils.symbolRowClick);
            });
        },
